...
const config = {
  headers: {
    ...
    Authorization:
      initialState.authentication.token ||
      "Bearer 369|TrCJ7t86NSUiPxoikvdUoZw1Ri4aEdCbcyMpUxDtf9275fa6",
...
export const loginDispatch = (bodyData) => async (dispatch) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      dispatch(
        loginFail(errorData.message || "An error occurred during login.")
      );
...
export const { createProduct, createProductSuccess, createProductFail } =
  actions;
...
export const { deleteProductRequest, deleteProductSuccess, deleteProductFail } =
  actions;
...
export const { createCategory, createCategorySuccess, createCategoryFail } =
  actions;
...
export const createListShopDispatch = (bodyData) => async (dispatch) => {
  try {
    ...
    fetch(`${baseURL}admin/add-shop-event`, requestOptions).then((data) => {
      console.log("data", data);
      if (data.status == 200) {
        Swal.fire({
          title: "Shop List Status",
