...
const TextEditor = () => {
  ...
  //     if(originalData[i].content!==data[i]){
  //       handleUpdateClick(originalData[i].id, data[i]);
  //     }
  //   }
  // }
  ...
  const handleUpdateClick = async () => {
    try {
      ...
      const response = await fetch(`https://loofer.bellazza.in/api/admin/privacy_policy/update`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
