...
const TextEditor = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch('https://loofer.bellazza.in/api/admin/term_and_condition', {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        // const privacy_data = result[0].map((ele) => `<p><strong>${ele.content}</strong></p><br>`).join('');
  ...
  const handleUpdateClick = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://loofer.bellazza.in/api/admin/term_and_condition/update', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
      ...
      if (response.ok) {
        Swal.fire({
          title: 'Terms & Condition Update Successful',
          icon: 'success'
        });
        // Redirect after 3 seconds
        setTimeout(() => {
          // You may use React Router's useHistory or any other method to redirect
          window.location.href = '/dashboard/termcondition';
        }, 3000);
      } else {
        // Handle error
        console.error('Terms & Condition Update Failed:', response.statusText);
        Swal.fire({
          title: 'Terms & Condition Update Failed',
          icon: 'error'
        });
      }
    } catch (error) {
      console.error('Terms & Condition Update Failed:', error.message);
      Swal.fire({
        title: 'Terms & Condition Update Failed',
        icon: 'error'
      });
