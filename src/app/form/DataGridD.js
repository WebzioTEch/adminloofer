...
export default function DataGridD() {
          ...
          <Box style={{ display: 'flex', justifyContent: 'space-between' }}>
            <button
              ...
              style={{
                ...
                height: '32px',
                width: '80px',
                backgroundColor: !blockIds.includes(params.id) ? 'grey' : 'red',
                color: 'white'
              }}
              onClick={() => {
                if (blockIds.includes(params.id)) {
                  const newBlockIds = blockIds.filter((e) => e !== params.id);
                  setBlockIds(newBlockIds);
                } else {
                  setBlockIds([...blockIds, params.id]);
                }
              }}
            >
              {blockIds?.includes(params.id) ? 'UnBlock' : 'Block'}
            </button>
          </Box>
        );
      }
  ...
  const mapCategoryDataToRows = (data) => {
    ...
    if (data && Array.isArray(data) && data.length > 0) {
      ...
      return [{ id: 1, name: 'Snow', email: 'Jon', created_at: '' }];
    }
  };
